...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.y = y
        self.total += 1
        self.distance += sqrt((self.x - xi) ** 2 + (self.y - yi) ** 2)
    ...
    def get_distance_from(self, self2):
        return sqrt((self2.x - self.x) ** 2 + (self2.y - self.y) ** 2)

    def get_distance_from_start(self):
        return sqrt((self.x - self.xi) ** 2 + (self.y - self.yi) ** 2)

    def get_distance_traveled(self):
        return self.distance

    def closer_to(self, p2, p3):
        p2c = sqrt((p2.x - self.x) ** 2 + (p2.y - self.y) ** 2)
        p3c = sqrt((p3.x - self.x) ** 2 + (p3.y - self.y) ** 2)
    ...
    def halfway_to(self, p2):
        ...
        return Point(self.x + phx, self.y + phy)
